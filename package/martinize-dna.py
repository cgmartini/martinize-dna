...
""",
    ...
    ("-o",        Option(str,                      1,     "cg.top", "Output topology (TOP)")),
...
# Fij = Fc exp( -a (rij - lo)**p )
    ...
    ("-dnatype",  Option(str,                      1,     'ss', "Type of DNA topology (ss/ds-soft/ds-stiff/ss-stiff) to create. (default: ds)")),
...
class CoarseGrained:
    ...
    residue_bead_names_polBB = spl("DIP BB DIM DBB SC1 SC2 SC3 SC4")
...
def breaks(residuelist,selection=("N","CA","C","P","C2'","C3'","O3'","C4'","C5'","O5'", "OP1", "OP2"),cutoff=4.0):
        ...
        for residue,rss,resname in zip(self.residues,self.sstypes,self.sequence):
            ...
            try:
                ...
                if self.options['ForceField'].ca2bb == "polBB" and residue[0][2] == self.residues[-1][0][2]:
                    ...
                    beads      = zip(CoarseGrained.residue_bead_names_polBB[:-2],beads,ids)
                elif self.options['ForceField'].ca2bb == "polBB" and residue[0][2] == self.residues[0][0][2]:
                    beads,ids = beads[1:],ids[1:]
                    beads      = zip(CoarseGrained.residue_bead_names_polBB[1:],beads,ids)
                elif self.options['ForceField'].ca2bb == "polBB":
                    beads      = zip(CoarseGrained.residue_bead_names_polBB,beads,ids)
                else:
